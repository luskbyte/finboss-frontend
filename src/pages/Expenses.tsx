...
import { Calendar, Pencil, Plus, Trash2, TrendingDown, Wallet } from "lucide-react";
...
type ExpensePayload = Omit<Expense, "id" | "created_at" | "updated_at">;
...
export default function ExpensesPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: ExpensePayload }) => expenseApi.update(id, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["expenses"] });
      qc.invalidateQueries({ queryKey: ["dashboard"] });
      closeModal();
  ...
  const recurringCount = data.filter((expense) => expense.recurring).length;
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-text-muted text-[13px]">
            Controle suas saídas e mantenha o orçamento sob acompanhamento.
          </p>
        </div>
        <button
      ...
      <div className="bg-surface border border-border rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.4)] overflow-hidden">
        {isLoading ? (
          <div className="p-10 text-center text-text-muted">Carregando despesas...</div>
        ) : data.length === 0 ? (
          <div className="py-14 text-center text-text-muted">
            ...
            <p className="text-sm mt-1">Cadastre seus gastos para acompanhar o orçamento.</p>
          </div>
        ) : (
                ...
                {data.map((expense) => (
                  <tr key={expense.id} className="border-t border-border">
                    ...
                    <td className="px-4 py-3">
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold tracking-wide bg-red/15 text-red">
                    ...
                    <td className={`px-4 py-3 ${expense.recurring ? "text-yellow" : "text-text-muted"}`}>
            ...
            <div className="px-5 py-4 border-b border-border flex items-center justify-between">
              <div>
                <h2 className="font-semibold text-lg">
                  {modal === "edit" ? "Editar despesa" : "Nova despesa"}
                </h2>
                <p className="text-sm text-text-muted">
                  Preencha os dados da saída financeira.
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <label className="flex items-center gap-2 rounded-lg border border-border bg-surface2 px-3 py-2 mt-6 md:mt-0">
                  ...
                  />
                  <span className="text-sm text-text-muted">Despesa recorrente</span>
                </label>
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                >
                  {isSubmitting ? "Salvando..." : modal === "edit" ? "Salvar alterações" : "Cadastrar despesa"}
                </button>
              </div>
            </form>
          </div>
